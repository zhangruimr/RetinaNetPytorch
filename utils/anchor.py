...
def layerAnchor(size, stride, baseAnchor):
    ...
    return layer_anchor
def generateAnchor(rpnFeaturemaps, oriSize = 608, baseSize=16, ratios=[0.5, 1, 2], scales=[2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0)]):
    base_anchor = baseAnchor(baseSize, ratios, scales)
    all_anchor = []
    for layer in rpnFeaturemaps:
        size = layer.shape
...
if __name__ == "__main__":
    anchor = baseAnchor()
    all_anchor = layerAnchor((3, 3, 512, 512), 2, anchor)
    print(all_anchor.shape)
    """
    pic = cv2.imread("../test.jpg")
    pic = cv2.resize(pic, (512, 512))
    anchor = baseAnchor()
    anchor = layerAnchor((1, 3, 32, 32), 16, anchor)
    #print(len(anchor))
    #cv2.imshow("win", pic)
    i = 0
    for box in anchor:
